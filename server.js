...
io.on('connection', socket=>{
    ...
    function pairing(){
        if(socketId.length > 1){
            ...
            console.log("Socket Array after delete of socket.id :" + socketId + "  Size :"+socketId.length);
            ...
            const x="you are now connected to a stranger";
            socket.to(socketPair[socket.id]).emit('waiting', x);
            socket.emit('waiting', x);
        }
    };
